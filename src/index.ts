...
import { init, runAgent, status, render, statusJson } from './cli/commands/index.js';

const program = new Command();

program
  .name('procside')
  .description('Process-first agent collaboration framework - documents AI agent workflows as structured process artifacts')
  .version('0.1.0');
...
program
  ...
  .option('-p, --path <path>', 'Project path', process.cwd())
  .option('-s, --status <status>', 'New status: pending, in_progress, completed, skipped, failed')
...
program
  ...
  .action(async (options) => {
    const { loadConfig, writeConfigFile, configExists } = await import('./config.js');
    ...
    if (options.set) {
      ...
      const updated = { ...config, [key]: value === 'true' ? true : value === 'false' ? false : value };
  ...
  .action(async (options) => {
    ...
    if (options.json) {
      console.log(JSON.stringify({
        ...
        errors: result.errors.map(e => ({ id: e.gate.id, message: e.message })),
        warnings: result.warnings.map(w => ({ id: w.gate.id, message: w.message })),
...
program
  ...
  .action(async (options) => {
    ...
    if (options.json) {
      console.log(JSON.stringify(gates.map(g => ({
        id: g.id,
        name: g.name,
    ...
    gates.forEach(g => {
      const enabled = config.qualityGates.gates.find(c => c.id === g.id)?.enabled ?? false;
